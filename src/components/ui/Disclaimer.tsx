...
export function Disclaimer({ className = '', variant = 'default' }: DisclaimerProps) {
    ...
    return (
      <Alert className={`bg-yellow-50/70 border-yellow-100 py-2 ${className}`}>
        ...
        <AlertDescription className="text-xs text-yellow-700">
          PathologyAI Hub provides AI-generated insights. Always consult healthcare professionals for medical advice.
        </AlertDescription>
      </Alert>
    );
  }
  ...
  return (
    <Alert className={`bg-yellow-50/70 border-yellow-100 py-3 ${className}`}>
      <AlertTriangle className="h-3.5 w-3.5 text-yellow-500" />
      <AlertTitle className="text-yellow-700 font-medium text-sm">PathologyAI Hub Disclaimer</AlertTitle>
      <AlertDescription className="text-yellow-700 mt-1 text-xs">
        <p>PathologyAI Hub is designed to assist with medical insights, education, and research.</p>
